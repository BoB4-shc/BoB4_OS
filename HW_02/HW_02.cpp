...
bool read_file_using_memory_map()
{
	...
	{
		printf("err ] no file exists. file = %ws", file_name);
		return false;
	}
...
#pragma warning(disable: 4127)
	do
	{
		...
		{
			printf("err ] GetFileSizeEx( %ws ) failed. gle = %u", file_path, GetLastError());
			break;
		}
		...
		if (fileSize.HighPart > 0)
		{
			printf("err ] file is too large to map. file = %ws, size = %llu", file_path, fileSize.QuadPart);
			break;
		}
		...
		if (ctx->view == NULL)
		{
			printf("err ] MapViewOfFile( %ws ) failed. gle = %u", file_path, GetLastError());
			break;
		}
...
)
{
	...
	{
		printf("err ] open_map_context() failed.");
...
bool bob_create_very_big_file(uint32_t size){
	HANDLE fp = NULL;
	DWORD readn;
	BOOL brtv;
	char read[1024];
	...
	// current dir \\ bob3.txt 파일명 생성
	wchar_t file_name_3[260];
	if (!SUCCEEDED(StringCbPrintfW(
		file_name_3,
		sizeof(file_name_3),
		L"%ws\\bob3.txt",
	...
	// current dir \\ bob4.txt 파일명 생성
	wchar_t file_name_4[260];
	if (!SUCCEEDED(StringCbPrintfW(
		file_name_4,
		sizeof(file_name_4),
		L"%ws\\bob4.txt",
	...
	create_very_big_file(L"BOB.txt", 1);

	StopWatch sw1 = StopWatch();
	StopWatch sw2 = StopWatch();

	sw1.Start();
	_ASSERTE(file_copy_using_memory_map(L"BOB.txt", L"BOB_mm.txt"));
	sw1.Stop();
	printf("file_copy_using_memory_map : %f ms\n", sw1.GetDurationMilliSecond());


	sw2.Start();
	_ASSERTE(file_copy_using_read_write(L"BOB.txt", L"BOB_rw.txt"));
	sw2.Stop();
	printf("file_copy_using_read_write : %f ms\n", sw2.GetDurationMilliSecond());
